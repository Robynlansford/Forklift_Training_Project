...
function mountScrollWorld(container, config) {
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      let outside = 0;
      // The track is deliberately 1vh taller than the final segment (see
      // measure(): "+1vh so the last flight completes"). Applying the normal
      // out-fade across that tail drops every scene to opacity 0 while the last
      // section's copy is written to HOLD its CTA — leaving the closing headline
      // and buttons floating over an empty background. So the final segment
      // never fades out; it holds full opacity to the end of the track.
      const isLastSeg = i === NSEG - 1;
      if (y < s.start) outside = s.start - y;
      else if (y > s.end && !isLastSeg) outside = y - s.end;
...
function injectCSS() {
  ...
  /* No 'background' here on purpose: this rule targets html/body, which are
     ANCESTORS of .sw-root, so a --sw-bg set on .sw-root can never be seen by
     it (custom properties only cascade to descendants) — it would always
     paint the hardcoded #F5EDE0 fallback instead. Worse, this <style> tag is
     injected into <head> once and never removed, so on a client-side route
     change away from the page that mounted the engine, the rule would keep
     tinting every other route's real body background. .sw-sky (a proper
     descendant of .sw-root) already paints the correct scoped background. */
  ...
  /* Same reasoning as .sw-route__label — the nav pill is a light translucent
     surface, so tab text is black rather than the dark-theme --sw-ink. */
  .sw-nav__item{font:inherit;font-size:.82rem;color:#000;border:0;background:transparent;cursor:pointer;padding:7px 14px;border-radius:999px;transition:color .25s,background .25s;}
  .sw-nav__item:hover{color:#000;} .sw-nav__item.is-active{color:#000;background:var(--sw-accent);}
  ...
  /* Black label text: the pill sits on an 85%-white background, so inheriting
     --sw-ink (near-white on this dark-themed site) rendered white-on-white. */
  .sw-route__label{position:absolute;right:24px;top:50%;transform:translateY(-50%) translateX(6px);white-space:nowrap;font-size:.78rem;font-weight:600;color:#000;background:color-mix(in srgb,#fff 85%,transparent);backdrop-filter:blur(6px);padding:5px 11px;border-radius:999px;opacity:0;pointer-events:none;transition:opacity .25s,transform .25s;border:1px solid color-mix(in srgb,var(--sw-accent) 14%,transparent);}
